...
d4 = copy(d3)
tmp = d4[, .SD[.N], by=Patient.ID]
tmp[, YTime := YTime + Time1 - Time0] 
for(v in c("Time", "FVC", "mRSS", "type", "A")){
  tmp[, (paste0(v,"0")) := get(paste0(v,"1"))]
  tmp[, (paste0(v,"1")) := get(paste0(v,"2"))]
  tmp[, (paste0(v,"2")) := NA]
}

d4 = rbind(tmp, d3)
d4 = d4[order(Patient.ID, Time0),]
