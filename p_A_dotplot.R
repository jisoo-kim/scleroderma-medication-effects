...
d[, fup := Time[.N], by = Patient.ID]
d[, sup := Time[1], by = Patient.ID]

mv = c("Pred","MTX","MMF","CTX","IVIG","AZA","Rituximab","Tocilizumab","HCQ","TNF","LEF")
pd = d[med == 1, c("Patient.ID", "Time", "fup", "sup", mv),with=F]
...
pd = pd[, c("Patient.ID", "Time", "fup", "sup", "type"), with=F]
...
for( i in 1:pncol ){
  ...
  plot(pd$Time, pd$id, xlab="", ylab="",  yaxt='n', axes = F, xlim = c(0,max(pd$Time,na.rm=T)+4),
